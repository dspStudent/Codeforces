...
public class ColoringGame {
	...
	static int lb(int a[], int start, int end, int key){
		while(start<end){
	...
	static int ub(int a[], int start, int end, int key){
		while(start< end){
			int mid=start+(end-start)/2;
			if(key>=a[mid]){
				start=mid+1;
    ...
    public static void main(String[] args) throws Exception {
        ColoringGame o=new ColoringGame();
            ...
            long count=0;
           	...
           	for(int i=0;i<n-2;i++){
           		for(int j=i+1;j<n-1;j++){
           			int ind=lb(a, j+1, n, a[i]+a[j]);

           			// println("i "+i+" j "+j+" a[i] "+a[i]+" a[j] "+a[j]+" sum "+(a[i]+a[j])+" ind "+ind+" a[ind] "+a[ind]);
       				int l=ub(a, j+1, ind+1, a[n-1]-a[i]-a[j]);	
