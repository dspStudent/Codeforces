...
public class ConcertTickets {
	...
	public void solve(List<Integer> ti, List<Integer> cu) {
		TreeMap<Integer, Integer>map = new TreeMap<>();
		for (int i : ti)map.put(i, map.getOrDefault(i, 0) + 1);
		for (int i : cu) {
			Map.Entry<Integer, Integer> entry = map.floorEntry(i);
			if(entry==null){
				op.append(-1).append("\n");
			}else {
				op.append(entry.getKey()).append("\n");
	...
	public static void main(String[] args) throws IOException {
		ConcertTickets o = new ConcertTickets();
		st = nst(br.readLine());
		int n = sToInt(st.nextToken()), m = sToInt(st.nextToken());
		List<Integer>ti = nextIntL(n);
		List<Integer>cu = nextIntL(m);
		o.solve(ti, cu);
		pw.println(op);
		pw.flush();
	}
