...
public class FillingBookcaseShelves {
	...
	public int minHeightShelves(int[][] a, int k) {
		int n=a.length, h=0;
		for(int i=0;i<n;i++)h=Math.max(h, a[i][1]);
		dp=new int[n][h+1][k+1];
		for(int i[][]:dp){
	...
	public static void main(String[] args) throws IOException {
		FillingBookcaseShelves o = new FillingBookcaseShelves();
		int t = sToInt(br.readLine());
		while (t-- > 0) {
			int n = sToInt(br.readLine());
			int a[][] = new int[n][2];
			for (int i = 0; i < n; i++)a[i] = nextIntA(2);
			int k = sToInt(br.readLine());
			op.append(o.minHeightShelves(a, k)).append("\n");
		}
		pw.println(op);
		pw.flush();
	}
