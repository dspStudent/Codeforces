...
public class StickLengths {
	...
	public long solve(List<Integer>a, int min, int max) {
		int n = a.size();
		...
		while (start <= end) {
			...
			int mid1=mid-1;
			int mid2=mid+1;
			long left=run(a, mid1);
			long right=run(a, mid2);
			long center=run(a, mid);
			// op.append(mid+" "+left+" "+center+" "+right).append("\n");
	...
	public long solve1(List<Integer>a, int min, int max) {
		int n = a.size();
		int start=min, end=max;
		while(start<=end){
	...
	public long run(List<Integer>a, int mid) {
		long sum=0L;
		int n=a.size();
		for(int i=0;i<n;i++){
			sum+=(long)Math.abs(mid-a.get(i));
		}
		return sum;
	}
	...
	public static void main(String[] args) throws IOException {
		StickLengths o = new StickLengths();
		int n = sToInt(br.readLine());
		st=nst(br.readLine());
		List<Integer>a=new ArrayList<>();
		int max=0, min=Integer.MAX_VALUE;
		for(int i=0;i<n;i++){
			int x=sToInt(st.nextToken());
			max=Math.max(max, x);
			min=Math.min(min, x);
			a.add(x);
		}
		op.append(o.solve(a, min, max)).append("\n");
		pw.println(op);
		pw.flush();
	}
