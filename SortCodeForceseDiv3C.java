import java.io.*;
import java.util.*;
public class SortCodeForceseDiv3C {
    ...
    public int solve(char a[], char b[], int[][]q){
    	int ans=0;
    	int n=q.length;
    	HashMap<Character, List<Integer>>mapa=new HashMap<>();
    	HashMap<Character, List<Integer>>mapb=new HashMap<>();
    	for(int i=0;i<a.length;i++){
    		char aa=a[i], bb=b[i];
    		if(!mapa.containsKey(aa)) mapa.put(aa, new ArrayList<>());
    		if(!mapa.containsKey(bb)) mapa.put(bb, new ArrayList<>());
    		mapa.get(aa).add(i);
    		mapb.get(bb).add(i);
    	}
    	op.append(mapa+" "+mapb).append("\n");
        return ans;
    }
    ...
    public static void main(String[] args) throws IOException{ 
        SortCodeForceseDiv3C o=new SortCodeForceseDiv3C();
        int t = sToInt(br.readLine());
        while(t-- > 0) {
            st=nst(br.readLine());
            int n=sToInt(st.nextToken()), qn=sToInt(st.nextToken());
            ...
            op.append(o.solve(a, b, q)).append("\n");
        }
        pw.println(op);
        pw.flush();
}
