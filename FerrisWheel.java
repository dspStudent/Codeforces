...
public class FerrisWheel {
	...
	public int solve(List<Integer>a, int x) {
		int n = a.size();
		Collections.sort(a);
		int count = 0, i=0, j=n-1;
		while(i<=j){
			if(a.get(i)+a.get(j)<=x && i!=j){
				count++;i++;j--;
			}else{
				count++;
				j--;
	...
	public static void main(String[] args) throws IOException {
		FerrisWheel o = new FerrisWheel();
		st = nst(br.readLine());
		int n = sToInt(st.nextToken()), x = sToInt(st.nextToken());
		List<Integer>a = nextIntL(n);
		op.append(o.solve(a, x)).append("\n");
		pw.println(op);
