...
public class ProgressiveSquare {
	...
	public void solve(List<Integer>a, int d, int r) {
		// Collections.sort(a);
		...
		int mat[][] = new int[m][m];
		for (int i = 0; i < m; i++)Arrays.fill(mat[i], -1);
		mat[0][0] = min;
		for (int i = 0; i < m; i++) {
			for (int j = 0; j < m; j++) {
				if (i + 1 < m) {
					int down = mat[i][j] + d;
					if (!map.containsKey(down) && mat[i+1][j]==-1) {
						// print2D(mat);
						op.append("No").append("\n");
						return;
					}
					if(mat[i+1][j]!=-1)continue;
					if (map.get(down) == 1)map.remove(down);
					else map.put(down, map.get(down) - 1);
					mat[i+1][j]=down;
				}
				if (j + 1 < m) {
					int right = mat[i][j] + r;
					if (!map.containsKey(right) && mat[i][j+1]==-1) {
						// print2D(mat);
						op.append("No").append("\n");
						return;
					}
					if(mat[i][j+1]!=-1)continue;
					if (map.get(right) == 1)map.remove(right);
					else map.put(right, map.get(right) - 1);
					mat[i][j+1]=right;
				}
	...
	public static void main(String[] args) throws IOException {
		ProgressiveSquare o = new ProgressiveSquare();
		int t = sToInt(br.readLine());
		while (t-- > 0) {
			st = nst(br.readLine());
			int n = sToInt(st.nextToken()), down = sToInt(st.nextToken()), right = sToInt(st.nextToken());
			List<Integer>a = nextIntL(n * n);
			o.solve(a, down, right);
		}
		pw.println(op);
		pw.flush();
	}
