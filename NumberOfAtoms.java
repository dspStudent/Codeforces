...
public class NumberOfAtoms {
	...
	public String solve(String s) {
		...
		for (int i = n - 1; i >= 0; i--) {
			char c = s.charAt(i);
			if (c == '(') {
				int mulCount = st.isEmpty() ? 0 : st.pop();
				temp = muliTheString(temp, mulCount);
				if (st.isEmpty()) {
					str.push(temp);
					temp = "";
				}
	...
	public String muliTheString(String s, int times) {
		if (times == 0)return s;
		String ans = "";
	...
	public String countOfAtoms(String s) {
		...
		for (int i = n - 1; i >= 0; i--) {
			...
			if (c >= 'A' && c <= 'Z') {
				chars = c + "" + chars;
				map.put(chars, map.getOrDefault(chars, 0) + sToIntNull(num));
				chars = "";
				num = "";
				continue;
			}
		...
		for (String ch : map.keySet()) {
			String in = map.get(ch) == 1 ? "" : map.get(ch) + "";
			ans += ch + "" + in + "";
		}
		return s;
	}

	public int sToIntNull(String s) {
		if (s.equals(""))return 1;
		else return Integer.parseInt(s);
	}

	// K4(N(G7))3(ON(SO3)2)2

	// stack 1:-
	// stack 2:- SOa3SO3ONSO3SO3ON, G7NG7NG7N,K4
	// num:   :-
	// string :-
	...
	public static void main(String[] args) throws IOException {
		NumberOfAtoms o = new NumberOfAtoms();
		int t = sToInt(br.readLine());
		while (t-- > 0) {
			String s = (br.readLine());
			op.append(o.countOfAtoms(s)).append("\n");
		}
		pw.println(op);
		pw.flush();
	}
