...
public class StrongPassword {
    ...
    public String solve(String s){
    	int n=s.length();
    	for(int i=1;i<n;i++){
    		char c=(char)((s.charAt(i)+1)%('z'-'a')+'a');
    		if(s.charAt(i-1)==s.charAt(i))return s.substring(0, i)+c+s.substring(i, n);
    	}
        return s+""+(char)((s.charAt(n-1)+1)%('z'-'a')+'a');
    }
    ...
    public static void main(String[] args) throws IOException{ 
        StrongPassword o=new StrongPassword();
        int t = sToInt(br.readLine());
        while(t-- > 0) {
            String s=br.readLine().trim();
            op.append(o.solve(s)).append("\n");
        }
        pw.println(op);
        pw.flush();
}
